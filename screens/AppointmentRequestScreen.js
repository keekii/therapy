...
const AppointmentRequestScreen = ({ navigation }) => {
  const { state, getAppointmentRequestList } = useContext(CalendarContext);
  ...
  const renderAppointmentRequestItems = (item, index) => {
    return <TouchableOpacity onPress={() => navigation.navigate('AppointmentRequestDetail', { data: item })}>
      <View style={styles.box}>
        <Image style={styles.image} source={{ uri: item['patient']['profile_pic'] }} />
  ...
  const renderRequestStatusItem = (status) => {
    switch (status) {
      case 'pending': return <View style={styles.box5}>
        <Text style={styles.status}>{item['status']}</Text>
      </View>;
      case 'accepted': return <View style={styles.box3}>
      <Text style={styles.status}>{item['status']}</Text>
    </View>;
    case 'rejected': return <View style={styles.box4}>
    <Text style={styles.status}>{item['status']}</Text>
  </View>;
    }
  }
